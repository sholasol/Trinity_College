...
if($user->isLoogedIn()){
    ...
    $email = escape($user->data()->email);
    $grp = escape($user->data()->group);
    $ref = escape($user->data()->reff_link);
}else{
    //Redirect::to('index');
    echo  " <script> window.location='../index'</script>";
}
...
$fstd->execute([$email]);
if($fstd->rowcount()>0){$freg = $fstd->fetch()->fstd;}else{$freg = 0;}
...
$atstd = dbConnect()->prepare("SELECT count(u.id) as atstd, u.email,u.phone,s.email,s.phone,s.reff_by,u.status FROM users.u INNER JOIN student.s ON u.email=s.email WHERE s.reff_by=? AND u.status=?");
$atstd->execute([$email,'Active']);
...
<aside id="leftsidebar" class="sidebar">
    <div class="navbar-brand">
        ...
        <a href="index.html"><img src="../../assets/img/logo.png" width="25" alt="Aero"><span class="m-l-10">Trinity-Affiliate</span></a>
    </div>
    <div class="menu">
        <ul class="list">
            <li>
            ...
            <li><a href="affiliate-user-profile"><i class="zmdi zmdi-account"></i><span>Our Profile</span></a></li>
            <li><a href="affiliate-student"><i class="zmdi zmdi-apps"></i><span>Student</span></a></li>
            <li><a href="affiliate-members"><i class="zmdi zmdi-account-box"></i><span>Affiliate</span></a></li>
