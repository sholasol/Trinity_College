...
//Exam Code
  ...
  if (is_uploaded_file($_FILES ['evidence'] ['tmp_name'])) {
      ...
      $imgFile = $_FILES['evidence']['name'];
        ...
        //Getting student ID
              $stdID = dbConnect()->prepare("SELECT id FROM users WHERE studentcode='$cd'");
              $stdID->execute();
              $rs = $stdID->fetch();
              $u_id = $rs['id'];
      ...
      //processing payment
      $pym =  dbConnect()->prepare("INSERT INTO payment SET 
     ...
     ");
        ...
        if($pym->execute()){
            ...
            $upd = dbConnect()->prepare("UPDATE student SET  status='Paid' WHERE studentcode='$cd'");
            $upd->execute();
            
            
           $e="Operation successful!"; 
             echo  " <script>alert('$e'); window.location='view?eml=$cd'</script>";
