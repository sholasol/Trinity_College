...
// }
 ...
 if(isset($_GET['delete'])){
     ...
     if($del){
         ...
         $dell->execute();
         if($dell){
         echo  " <script> alert('Operation Successful');window.location='astudent'</script>";
         }
     }
 }
?>
    ...
    <div class="block-header">
        <div class="row">
            <div class="col-lg-7 col-md-6 col-sm-12">
                <h2>Student</h2>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="newStudent"><i class="zmdi zmdi-home"></i> New Student</a></li>
                </ul>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover c_table theme-color dataTable js-exportable">
                                ...
                                <?php 
                                    ...
                                    $user = DB::getInstance()->query("SELECT * FROM student WHERE (reff_by IS NOT NULL) AND reff_by !='' AND reff_by !=0  ORDER BY id DESC");
                                    if($user ->error()){
                                       ...
                                       foreach($user->results() as $std){
                                           ...
                                           $affl = $std->reff_by;
                                           // Affliate
                                           $dbx = dbConnect()->prepare("SELECT fname, lname, phone FROM users WHERE code='$affl'");
                                           $dbx->execute();
                                           $ry = $dbx->fetch();
                                           $aname = $ry['fname'].' '.$ry['lname'];
                                           $aphone = $ry['phone'];
                                    ...
                                    <tr>
                                        ...
                                        <td> <?php echo $name; ?> </td>
                                        <td> <?php echo $aname.'<br> ('.$aphone.')'; ?> </td>     
