...
if(isset($_POST['submit'])){
    ...
    echo  " <script>alert('Please fill in the child's first name'); window.location='edit?code=$stdnt'</script>";
    }elseif(empty($_POST['lname'])){
    
    echo  " <script>alert('Please fill in the child's last name'); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['dob'])){
    
    echo  " <script>alert('Please fill in child's date of birth'); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['email'])){
    
    echo  " <script>alert('Please fill in your email or child's email); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['phone'])){
    
    echo  " <script>alert('Please fill in your phone number or child's phone number'); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['ffname'])){
    
    echo  " <script>alert('Please fill in fathers's full name'); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['fphone'])){
    
    echo  " <script>alert('Please fill in fathers's phone number'); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['femail'])){
    
    echo  " <script>alert('Please fill in fathers's email'); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['mfname'])){
    
    echo  " <script>alert('Please fill in mothers's full name'); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['memail'])){
    
    echo  " <script>alert('Please fill in mother's email '); location='edit?code=$stdnt'</script>";
    }
    elseif(empty($_POST['mphone'])){
    
    echo  " <script>alert('Please fill in mother's phone number'); location='edit?code=$stdnt'</script>";
    }else{
...
if($mode =="bank"){
    //update student information and redirect to upload payment evidence  
     //process student information without payment evidence
        $upd = dbConnect()->prepare("UPDATE student SET mname=:mn, gender=:gnd, status=:sts, ffname=:ffn, femail=:feml,
       ...
       $upd->bindParam(':excode', $scode);
       $upd->bindParam(':cen', $center);
       if($upd->execute()){
           
           
           //student payment
            $dbx = dbConnect()->prepare("SELECT amount FROM payment WHERE studentcode='$scode'");
            $dbx->execute();
            $ra = $dbx->fetch();
            $amt = $ra['amount'];
            
            if($amt > 0){
                $e="Student information has been successfully updated!."; 
                echo  " <script>alert('$e'); window.location='student'</script>";
            }else{
                $e="Your form has been submitted! Please upload the bank evidence."; 
                echo  " <script>alert('$e'); window.location='bank?code=$scode'</script>";
            }
