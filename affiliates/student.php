...
// }
 ...
 if(isset($_GET['delete'])){
     ...
     if($del){
         ...
         $dell->execute();
         if($dell){
         echo  " <script> alert('Operation Successful');window.location='student'</script>";
         }
     }
 }
?>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover c_table theme-color dataTable js-exportable">
                                ...
                                <?php 
                                    ...
                                    $user = DB::getInstance()->query("SELECT * FROM student WHERE status='Paid' or status='Approve' ");
                                    if($user ->error()){
                                       ...
                                       foreach($user->results() as $std){
                                           ...
                                           $dbx = dbConnect()->prepare("SELECT * FROM payment WHERE studentcode='$scode'");
                                           $dbx->execute();
                                           $ra = $dbx->fetch();
                                           $mod = $ra['mode'];
                                           $amt = $ra['amount'];
                                    ...
                                    <tr>
                                        ...
                                        <td> <?php echo $name; ?><br><small><?php echo $scode; ?></small> </td>
                                            ...
                                            <select onChange="window.location.href=this.value" class="form-control show-tick" >
                                               ...
                                               <!-- <option value="?approve=<?php echo $std->id; ?>"> Confirm</option>-->
                                                ...
                                                if($email =='marketing@trinityng.com'){
                                                    ...
                                                    echo "<option value='edit?code=$scode'>Edit</option>";
