...
foreach($Tstd->results() as $rst){$rst->tstd;}

$std = dbConnect()->prepare("SELECT count(id) as rstd FROM users WHERE role=?");
$std->execute(['Student']);
if($std->rowcount()>0){$rstd = $std->fetch()->rstd;}else{$rstd = 0;}

// Total User from database
$Tuser = DB::getInstance()->query("SELECT count(id) as Tuser FROM users");
foreach($Tuser->results() as $us){$us->Tuser;}

// Registered Admin from databse
$radm = dbConnect()->prepare("SELECT count(id) as radm FROM users WHERE role=?");
$radm->execute(['Admin']);
if($radm->rowcount()>0){$Radm = $radm->fetch()->radm;}else{$Radm = 0;}

// Total Active User from databse
$Auser = dbConnect()->prepare("SELECT count(id) as Auser FROM users WHERE status=?");
$Auser->execute(['Active']);
if($Auser->rowcount()>0){$Aus = $Auser->fetch()->Auser;}else{$Aus = 0;}
...
<aside id="leftsidebar" class="sidebar">
    <div class="navbar-brand">
        ...
        <a href="index.html"><img src="assets/images/logo.png" width="25" alt="Trinity College"><span class="m-l-10">Admin</span></a>
    </div>
    <div class="menu">
        <ul class="list">
            <li>
            ...
            <li><a href="profile"><i class="zmdi zmdi-account"></i><span>Our Profile</span></a></li>
            <li><a href="student"><i class="zmdi zmdi-apps"></i><span>Student</span></a></li>
