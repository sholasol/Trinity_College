            ...
            <div class="inner">
                ...
                if(isset($_POST['submit'])){
                    ...
                    if (strlen($phone_to_check) < 9 || strlen($phone_to_check) > 14) {
                        ...
                        //check if email is present in the user's table
                        $quy = dbConnect()->prepare("SELECT * FROM student WHERE email=? ");
                        $quy->execute([$email]);
                        $present = $quy->rowcount();
                        
                        //check if email is present in the student table
                        $query = dbConnect()->prepare("SELECT * FROM student WHERE email=? ");
                        $query->execute([$email]);
                        if($query->rowcount() >0 || $present > 0 ){
                            $e='This email already exist! Try login with your email and phone number';
                            echo "<div class='alert alert-info alert-dismissible fade show' role='alert'>
                                    <a href='' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                                    <strong>Oops! </strong> $e. <a href='student'> Login Here</a>
                                </div>";
                        }else{
                        $ins = dbConnect()->prepare("INSERT INTO student(fname,lname,email,phone,status,reff_by, created) VALUES (?,?,?,?,?,?,?)");
                            $ins->execute([$fname,$lname,$email,$phone, 'Unpaid',$reff_by, $date]);
                            if($ins){
                                //Registering student as user.
                                $ins = dbConnect()->prepare("INSERT INTO users(fname,lname,email,password,salt, phone,role,status, ref_by) VALUES (?,?,?,?,?,?,?,?,?)");
                                $ins->execute([$fname, $lname, $email, $pw, $salt, $phone,'Student','offline',$reff_by]);
                                ...
                                //Sending Email
                                 ...
                                 $msg ='Thank you for registering with Trinity International College, Ofada, Ogun State.To complete your registration on the admission portal, kindly login at  www.trinityng.com/student with your email and phone number' .'('.$email.', '. $phone.')';
                                  ...
                                  $message .= "<tr><td><strong>Regsitration Portal:</strong> </td><td>" .$link. "</td></tr>";
